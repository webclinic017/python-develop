...
class Database(object):
    def __init__(self, HOST="localhost", USER="root", PASSWORD="123456", database="data", charset="utf8",
                 autocommit=True):
        self.host = HOST
        self.user = USER
    ...
    def insert_klines(self, symbol, interval, klines):
        ...
        for i in range(length - 1):
            kline = klines[i]
            insert_sql += "('{}',{},{},{},{},{},{},{},{},{},{},{}),".format(self.to_datetime(kline[0]), kline[0],
                                                                            kline[1], kline[2], kline[3], kline[4],
                                                                            kline[5], kline[6], kline[7], kline[8],
                                                                            kline[9], kline[10])
        # print(length)
        kline = klines[length - 1]
        insert_sql += "('{}',{},{},{},{},{},{},{},{},{},{},{});".format(self.to_datetime(kline[0]), kline[0], kline[1],
                                                                        kline[2], kline[3], kline[4], kline[5],
                                                                        kline[6], kline[7], kline[8], kline[9],
                                                                        kline[10])
    ...
    def insert_trades(self, symbol, trades):
        ...
        for trade in trades:
            # print(trade)
            insert_sql = "insert into {} values('{}',{},{},{},{},{},{});".format(table_name,
                                                                                 self.to_datetime(trade["time"]),
                                                                                 trade["id"], trade["price"],
                                                                                 trade["qty"], trade["quoteQty"],
                                                                                 trade["time"], trade["isBuyerMaker"])
            self.cursor.execute(insert_sql)
        # print("Insert klines success")
    ...
    def batch_insert_trade(self, symbol, trades: list):
        ...
        for i in range(length - 1):
            ...
            if ("quote_qty" in trade.keys()):
                insert_sql += "('{}',{},{},{},{},{},{}),".format(self.to_datetime(int(trade["time"])), trade["id"],
                                                                 trade["price"], trade["qty"], trade["quote_qty"],
                                                                 trade["time"], trade["is_buyer_maker"])
            else:
                insert_sql += "('{}',{},{},{},{},{},{}),".format(self.to_datetime(int(trade["time"])), trade["id"],
    ...
    def insert_trade(self, symbol, trade):
        ...
        # print(trade)
        insert_sql = "insert into {} values('{}',{},{},{},{},{},{});".format(table_name,
                                                                             self.to_datetime(int(trade["time"])),
                                                                             trade["id"], trade["price"], trade["qty"],
                                                                             trade["quote_qty"], trade["time"],
    ...
    def count_trade(self, symbol, interval=10):
        ...
        while (True):
            sql = sql + (
                "count(case when qty>={} and qty<{} then 1 end) as \'{}-{}\',".format(start, start + interval, start,
                                                                                      start + interval))
            start += interval
            interval = interval * 2
            if (start >= 5000):
    ...
    def new_table_klines(self, symbol, interval):
        sql = "create table {}_KLINES_{}(time timestamp,Open_time bigint primary key,Open double,High double,Low double,Close double,Volume double,Close_time bigint,Quote_asset_volume double,count int,taker_buy_volume double,taker_buy_quote_volume double);".format(
            symbol, interval)
        self.cursor.execute(sql)
    ...
    def new_table_trades(self, symbol):
        sql = "create table {}_TRADE(time timestamp,id bigint primary key,price float,qty float,quoteQty float,ttime bigint,isBuyerMaker varchar(20));".format(
            symbol)
        self.cursor.execute(sql)
    ...
    def select_klines(self, symbol, interval, limit, startTimestamp=None):
        ...
        if (startTimestamp != None):
            ...
            sql = "select Open_time,Open,High,Low,Close,Volume,Close_time,Quote_asset_volume,taker_buy_volume from {} where Open_time>={} order by Open_time limit {}".format(
                table_name, startTimestamp, limit)
        else:
            sql = "select Open_time,Open,High,Low,Close,Volume,Close_time,Quote_asset_volume,taker_buy_volume from {} order by Open_time desc limit {}".format(
                table_name, limit)
        self.cursor.execute(sql)
        if (startTimestamp != None):
    ...
    def select_trade(self, symbol, minqty=300, desc=False, limit=100):
        if (desc == False):
            ...
            desc_f = "DESC"
        table_name = "{}_TRADE".format(symbol)
        sql = "select time,price,qty,quoteQty,isBuyerMaker from {} where qty>{} order by id {} limit {}".format(
            table_name, minqty, desc_f, limit)
        # print(sql)
        self.cursor.execute(sql)
        if (desc == True):
            data = reversed(self.cursor.fetchall())
    ...
    def get_tardes_limit(self, symbol, starttime, endtime, count):
        ...
        sql = "select ttime,price,qty,isBuyerMaker from {} where ttime>={} and ttime<{} order by qty desc limit {}".format(
            table_name, starttime, endtime, count)
        self.cursor.execute(sql)
    ...
    def select_trade_to_csv(self, symbol, minqty=100, limit=10000):
        table_name = "{}_TRADE".format(symbol)
        sql = "select ttime,price,qty,isBuyerMaker from {} where qty>{} order by ttime desc limit {}".format(table_name,
                                                                                                             minqty,
                                                                                                             limit)
        self.cursor.execute(sql)
        data = reversed(self.cursor.fetchall())
    ...
    def group_price_sum(self, symbol):
        sql = "select price,sum(qty),isBuyerMaker from {}_trade group by price,isBuyerMaker order by price desc;".format(
            symbol)
        self.cursor.execute(sql)
        results = self.cursor.fetchall()
        return results
